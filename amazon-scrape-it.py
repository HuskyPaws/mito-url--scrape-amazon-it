...
# Streamlit app title
st.title("Amazon Product Data Scraper - Italy")
...
def scrape_data(url, api_key, max_retries=5, initial_delay=2):
    ...
    while retries < max_retries:
        try:
            ...
            if byline_info:
                data_dict["Brand Store URL"] = "https://www.amazon.it" + byline_info[0].get('href')
                ...
                for element in response_json.get('data', []):
                    ...
                    # Handle detail bullets
                        ...
                        if not element.get('error') and element.get('results'):
                            ...
                            # Check for manufacturer with Italian terms
                            if "Produttore" in bullet_points:
                                try:
                                    produttore = bullet_points.split("Produttore")[1].strip().split("\n")[0].strip()
                                    data_dict["Manufacturer"] = produttore.split(":")[1].strip() if ":" in produttore else produttore.strip()
                                except (IndexError, KeyError) as e:
                                    logging.error(f"Error processing Produttore: {str(e)}")

                            # Check for model number with Italian terms
                            if "Numero modello articolo" in bullet_points:
                                try:
                                    numero_modello = bullet_points.split("Numero modello articolo")[1].strip().split("\n")[0].strip()
                                    data_dict["Item Model Number"] = numero_modello.split(":")[1].strip() if ":" in numero_modello else numero_modello.strip()
                                except (IndexError, KeyError) as e:
                                    logging.error(f"Error processing Numero modello: {str(e)}")
                    ...
                    # Handle tech specs table
                        ...
                        if element.get('results'):
                            ...
                            if "Produttore" in table_data and data_dict["Manufacturer"] == "Not found":
                                try:
                                    produttore = table_data.split("Produttore")[1].strip().split("\n")[0].strip()
                                    data_dict["Manufacturer"] = produttore.split("\t")[1].strip() if "\t" in produttore else produttore.strip()
                                except (IndexError, KeyError) as e:
                                    logging.error(f"Error processing table Produttore: {str(e)}")

                            # Check for model number in table
                            if "Numero modello articolo" in table_data and data_dict["Item Model Number"] == "Not found":
                                try:
                                    numero_modello = table_data.split("Numero modello articolo")[1].strip().split("\n")[0].strip()
                                    data_dict["Item Model Number"] = numero_modello.split("\t")[1].strip() if "\t" in numero_modello else numero_modello.strip()
                                except (IndexError, KeyError) as e:
                                    logging.error(f"Error processing table Numero modello: {str(e)}")
